...
function LeaderboardRow({ entry, rank, skipAnimation }: { entry: LeaderboardEntry; rank: number; skipAnimation: boolean }) {
  ...
  return (
    <div
      className={`flex items-center px-3 md:px-5 ${skipAnimation ? '' : 'lb-slide'}`}
      style={{
        height:         '52px',
    ...
    >
      <div className="w-6 md:w-8 flex-shrink-0">
        <span className="text-xs font-medium" style={{ color: isFirst ? 'var(--color-gold)' : 'var(--color-text-secondary)', fontFamily: 'JetBrains Mono, monospace' }}>
          {rank + 1}
        </span>
      </div>
      ...
      <div className="flex items-center gap-2 md:gap-3 w-32 md:w-56 flex-shrink-0">
        ...
        <span className="text-xs font-medium text-white truncate" title={formatName(entry.name)}>
      ...
      <div className="w-20 md:w-28 flex-shrink-0">
        <span className="text-xs font-medium" style={{ color: isFirst ? 'var(--color-gold)' : 'var(--color-text-primary)', fontFamily: 'JetBrains Mono, monospace' }}>
          ${entry.final_bankroll.toFixed(0)}
        </span>
      </div>

      <div className="flex-1">
        <span className="text-xs font-medium" style={{ color: profit >= 0 ? 'var(--color-gold)' : 'var(--color-loss-fg)', fontFamily: 'JetBrains Mono, monospace' }}>
...
export function LeaderboardView() {
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: 'var(--color-page)' }}>
      ...
      {/* ── Winner hero — full-width, announces the result immediately ── */}
      {winner && (
        <div
          ...
          aria-label={`Tournament winner: ${formatName(winner.name)}`}
        >
          ...
          <div className="flex items-center gap-4 md:gap-8 mt-1">
            <div className="flex flex-col items-center">
              <span className="text-[9px] md:text-[10px] tracking-[2px]" style={{ color: 'var(--color-text-secondary)' }}>FINAL BANKROLL</span>
              <span className="text-xl md:text-2xl font-medium" style={{ fontFamily: 'JetBrains Mono, monospace', color: winnerColor }}>
                ${winner.final_bankroll.toFixed(0)}
              </span>
            </div>
            <div style={{ width: '1px', height: '32px', background: 'var(--color-border)' }} aria-hidden="true" />
            <div className="flex flex-col items-center">
              <span className="text-[9px] md:text-[10px] tracking-[2px]" style={{ color: 'var(--color-text-secondary)' }}>NET PROFIT</span>
              <span
                className="text-xl md:text-2xl font-medium"
                style={{ fontFamily: 'JetBrains Mono, monospace', color: winner.net_profit >= 0 ? winnerColor : 'var(--color-loss-fg)' }}
              >
            ...
            <div style={{ width: '1px', height: '32px', background: 'var(--color-border)' }} aria-hidden="true" />
            <div className="flex flex-col items-center">
              <span className="text-[9px] md:text-[10px] tracking-[2px]" style={{ color: 'var(--color-text-secondary)' }}>POINTS</span>
              <span className="text-xl md:text-2xl font-medium" style={{ fontFamily: 'JetBrains Mono, monospace', color: winnerColor }}>
                {winner.points}
              </span>
            </div>
          ...
          <div className="flex gap-3 md:flex-col md:gap-4 overflow-x-auto pb-2 md:pb-0">
            {lb.slice(1, 3).map((entry, idx) => {
              ...
              return (
                <div
                  key={entry.name}
                  className={`flex items-center gap-3 md:gap-4 px-3 md:px-5 flex-shrink-0 ${skipAnimation ? '' : 'lb-slide'}`}
                  style={{
                    height:         '68px',
                    minWidth:       '200px',
                ...
                >
                  ...
                  <div className="flex flex-col items-end flex-shrink-0">
                    ...
                    <span className="text-[10px]" style={{ color: profit >= 0 ? eColor : 'var(--color-loss-fg)', fontFamily: 'JetBrains Mono, monospace' }}>
                      {profit >= 0 ? '+' : ''}${profit.toFixed(0)}
                    </span>
                  </div>
                </div>
        ...
        <tbody>
          {rounds.map(round => (
            <tr key={round.round_num} style={{ borderBottom: '1px solid var(--color-border-sub)' }}>
              ...
              <td className="px-3 md:px-4 py-2 text-[10px] md:text-xs" style={{ color: 'var(--color-text-secondary)', fontFamily: 'JetBrains Mono, monospace' }}>
